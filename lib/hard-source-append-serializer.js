...
const _readTable = _this => readFile(_tablepath(_this), 'utf8')
...
const _appendBlock = (_this, _table, blockContent, index, next) => {
  ...
  if (_this._fd !== null && index % _table.logSize === 0) {
    ...
    .then(() => _openLog(_this, 'a', _table, index));
  }
...
const _compactSize = (_this, _table) => Math.max(
...
class AppendSerializer {
  ...
  read(mustLock) {
    ...
    return _lock(_this, mustLock, promise => promise
    .then(() => _read())
    .catch(e => _closeLog(_this)
    .then(() => {
      throw e;
    })));
  }
      ...
      .then(_table => {
        ...
        function step(op, next) {
          ...
          if (content !== null) {
            ...
            const bulk = Array.from(new Array(blockCount))
            .map((_, i) => i);
