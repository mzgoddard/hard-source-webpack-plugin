...
HardBasicDependencyPlugin.prototype.apply = function(compiler) {
  compiler.plugin('--hard-source-freeze-dependency', function(frozen, dependency, extra) {
    if (dependency.constructor.name === 'CommonJsRequireDependency') {
      return {
    ...
    else if (dependency.constructor.name === 'RequireHeaderDependency') {
      return {
        type: dependency.constructor.name,
        range: dependency.range,
      };
    }
    console.log(dependency.constructor.name);
  ...
  let CommonJsRequireDependency, RequireHeaderDependency;
  compiler.plugin('after-plugins', function() {
    compiler.plugin('compilation', function(compilation) {
      const Dependencies = compilation.dependencyFactories.keys();
      for (const Dep of Dependencies) {
        if (Dep.name === 'CommonJsRequireDependency') {
          CommonJsRequireDependency = Dep;
        }
        if (Dep.name === 'RequireHeaderDependency') {
          RequireHeaderDependency = Dep;
        }
      }
      // console.log(Array.from(compilation.dependencyFactories.keys()).length, CommonJsRequireDependency, RequireHeaderDependency)
    });
  });

  compiler.plugin('--hard-source-thaw-dependency', function(dependency, frozen, extra) {
    if (frozen.type === 'CommonJsRequireDependency') {
      return new CommonJsRequireDependency(frozen.request, frozen.range);
    }
    else if (frozen.type === 'RequireHeaderDependency') {
