      ...
      .then(() => {
        ...
        Object.keys(moduleResolveCache).forEach(key => {
          ...
          if (resolveItem.type === 'context') {
            const contextMissing = missingCache.context[JSON.stringify([
              resolveKey.context,
            ...
            const normalMissing = missingCache[normalId][JSON.stringify([
              resolveKey[1],
              resolveItem.resource.split('?')[0]
            ])];
            ...
            resolveItem.loaders.forEach(loader => {
              ...
              let loaderMissing = missingCache.loader[JSON.stringify([
                resolveKey[1],
                loader.split('?')[0]
              ])];
              if (!loaderMissing) {
