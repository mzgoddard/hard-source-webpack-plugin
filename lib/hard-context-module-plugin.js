...
const build4 = function(options, compilation, resolver, fs, callback) {
  ...
  contextModuleFactory.hooks.afterResolve.callAsync(
    Object.assign(
      {
        resolveDependencies: contextModuleFactory.resolveDependencies.bind(
          contextModuleFactory,
        ),
      },
      this.cacheItem.constructor.options,
    ),
    (err, { resolveDependencies }) => {
      ...
      ContextModule.prototype.build.call(
        this,
        options,
        compilation,
        resolver,
...
const builtTime4 = ({ buildInfo, build }) =>
  buildInfo ? buildInfo.builtTime : build.buildInfo.builtTime;
...
const build3 = function(options, compilation, resolver, fs, callback) {
  const contextModuleFactory = this.__hardSource_contextModuleFactory;
  contextModuleFactory.applyPluginsWaterfallAsync(
    'after-resolve',
    Object.assign(
      {
...
const builtTime3 = ({ builtTime, build }) =>
  builtTime ? builtTime : build.builtTime;
...
class HardModuleContextPlugin {
  ...
  apply(compiler) {
    ...
    pluginCompat.tap(
      compiler,
      '_hardSourceMethods',
      'HardModuleContextPlugin copy methods',
      _methods => {
        methods = _methods;
    ...
    pluginCompat.tap(
      ...
      'HardModuleContextPlugin freeze',
      (frozen, module, { compilation }) => {
        ...
        if (
          ...
          ((frozen && builtTime(module) >= builtTime(frozen)) || !frozen)
        ) {
          if (module.cacheItem) {
          ...
          const m = serialContextModule.freeze(
            null,
            module,
            {
              module,
    ...
    pluginCompat.tap(
      ...
      'HardModuleContextPlugin thaw',
      (module, frozen, { compilation, contextModuleFactory }) => {
        if (frozen.type === 'ContextModule') {
          const m = serialContextModule.thaw(
            null,
            frozen,
            {
              state: { imports: {} },
