...
class HardContextModuleFactory {
  ...
  _create(context, dependency, callback) {
      ...
      resolveIdentifier = JSON.stringify({
        ...
        options: Object.assign({}, dependency.options, {
          regExp: dependency.options.regExp
            ? dependency.options.regExp.source
            : null,
        }),
      });
    ...
    const resolved =
      resolveCache[resolveIdentifier] || resolveCache[absResolveIdentifier];
      ...
      if (cacheItem && !cacheItem.invalid) {
        ...
        if (
          !HardContextModule.needRebuild(
            cacheItem,
            relateContext.contextNormalPath(
          ...
          return callback(
            null,
            fetch('Module', resolved.identifier, {
              compilation,
