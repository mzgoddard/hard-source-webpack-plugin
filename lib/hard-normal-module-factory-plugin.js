...
class NormalModuleFactoryPlugin {
  apply(compiler) {
    ...
    compilerHooks._hardSourceMethods.tap('HardSource - HardNormalModuleFactoryPlugin', function(methods) {
      fetch = methods.fetch;
    });

    compilerHooks.compilation.tap('HardSource - HardNormalModuleFactoryPlugin', function(compilation, params) {
      // if (!active) {return;}
      ...
      var normalModuleFactoryHooks = pluginCompat.hooks(params.normalModuleFactory);

      // Webpack 2 can use different parsers based on config rule sets.
      normalModuleFactoryHooks.parser.for('javascript/auto').tap('HardSource - HardNormalModuleFactoryPlugin', function(parser, options) {
      ...
      normalModuleFactoryHooks.resolver.tap('HardSource - HardNormalModuleFactoryPlugin', function(fn) {
        return function(request, cb) {
          ...
          var fromCache = function() {
            ...
            if (module) {;
              ...
              compilation.__hardSourceBuildHashes(buildInfo.fileDependencies, buildInfo.contextDependencies)
              .then(function() {
                cb(null, result);
              });
            }
      ...
      normalModuleFactoryHooks.createModule.tap('HardSourceWebpackPlugin', result => {
        if (
          compilation.cache &&
