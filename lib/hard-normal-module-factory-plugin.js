...
class NormalModuleFactoryPlugin {
  apply(compiler) {
    ...
    compilerHooks._hardSourceMethods.tap('HardSource - HardNormalModuleFactoryPlugin', methods => {
      fetch = methods.fetch;
    });

    compilerHooks.compilation.tap('HardSource - HardNormalModuleFactoryPlugin', (compilation, {normalModuleFactory, contextModuleFactory}) => {
      // if (!active) {return;}
      ...
      normalModuleFactoryHooks.parser.for('javascript/auto').tap('HardSource - HardNormalModuleFactoryPlugin', (parser, options) => {
      ...
      normalModuleFactoryHooks.resolver.tap('HardSource - HardNormalModuleFactoryPlugin', fn => (request, cb) => {
        ...
        const fromCache = () => {
          ...
          if (!result.parser || !result.parser.parse) {
            result.parser = result.settings ?
              normalModuleFactory.getParser(result.type, result.settings.parser) :
