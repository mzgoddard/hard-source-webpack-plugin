      ...
      compilation => {
        ...
        compilation.__hardSourceBuildHashes = (files, _contexts) =>
          Promise.all([
            files &&
              bulkFsTask(files, (file, task) => {
                ...
                compiler.inputFileSystem.readFile(
                  ...
                  task((err, body) => {
                    if (err) {
                      // Silently fail. Failure to read is treated as the file
                      // is missing. Webpack will handle the missing dependency
                      // if its a module with a module missing error in the
                      // build or the relevant loader or other system will
                      // create a relevant error.
                      fileMd5s[file] = '';
                      return;
                    }
            ...
            _contexts &&
              (() => {
                if (!Array.from(_contexts).some(ctx => fileMd5s[ctx])) {
                  return;
                }
                const contexts = contextStamps(_contexts, stats);
                return Promise.all(values(contexts)).then(() => {
                  for (const contextPath in contexts) {
                    const context = contexts[contextPath];
      ...
      (compilation, { relateNormalPath }) => {
        ...
        const writeMd5Ops = Promise.all(
          ...
          if (!compilation.compiler.parentCompilation) {
            ...
            // Reset after every top-level compiler build. Need to check md5's
            // every build.
            //
            // TODO: A race or other error producing conditions keeps setting
            // fileMd5s to an empty object from propagating to other systems.
            for (const key in fileMd5s) {
              delete fileMd5s[key];
            }
          }
        });
