...
class Md5Cache {
  apply(compiler) {
    ...
    function bindFS() {
      ...
      mtime = file => stat(file)
      .then(stat => +stat.mtime)
      .catch(() => 0);

      md5 = file => readFile(file)
      .then(contents => crypto.createHash('md5').update(contents, 'utf8').digest('hex'))
      .catch(() => '');
      ...
      contextStamp = (dir, stats) => {
        ...
        function walk(dir) {
          ...
          .then(items => Promise.all(items.map(item => {
            ...
            .then(stat => {
              if (stat.isDirectory()) {
                ...
                .then(items2 => items2.map(item2 => path.join(item, item2)));
              }
          ...
          .catch(() => [])
          .then(items => items.reduce((carry, item) => carry.concat(item), [])
          .filter(Boolean));
        }
      ...
      compilation.__hardSourceBuildHashes = (files, _contexts) => (
        Promise.all([
          files && bulkFsTask(files, (file, task) => {
            ...
            compiler.inputFileSystem.readFile(file, task((err, body) => {
              if (err) {
                fileMd5s[file] = '';
                return;
              }
    ...
    pluginCompat.tapPromise(compiler, '_hardSourceWriteCache', 'HardSource - Md5Cache', (compilation, {relateNormalPath}) => {
      ...
      const writeMd5Ops = Promise.all(Object.keys(buildingMd5s).map(key => buildingMd5s[key]))
      .then(() => {
