...
class HardContextModuleFactoryPlugin {
  ...
  apply(compiler) {
    ...
    pluginCompat.tap(compiler, 'afterPlugins', 'HardContextModuleFactoryPlugin', () => {
      pluginCompat.tap(compiler, 'compilation', 'HardContextModuleFactoryPlugin', (compilation, {contextModuleFactory}) => {
        const factories = compilation.dependencyFactories;
        const contextFactory = factories.get(RequireContextDependency) ||
