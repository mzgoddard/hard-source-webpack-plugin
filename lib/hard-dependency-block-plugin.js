...
const BlockSchemas4 = [
  ...
  ['ImportDependenciesBlock', 'request', 'range', 'chunkName', 'module', 'loc', 'originModule'],
  ['RequireEnsureDependenciesBlock', 'expr', 'successExpression', 'errorExpression', 'chunkName', 'chunkNameRange', 'module', 'loc'],
  ['AsyncDependenciesBlock', 'name', 'module', 'loc', 'request'],
];
...
function assertFrozen({length}, original, typeName, freeze) {
  if (length !== original.length) {
    ...
    if (didNotFreeze.length > 0) {
      throw new Error(`Unfrozen ${typeName}: ${didNotFreeze
...
class HardDependencyBlockPlugin {
  constructor(options) {
  ...
  apply(compiler) {
    ...
    pluginCompat.tap(compiler, '_hardSourceMethods', 'HardDependencyBlockPlugin', _methods => {
      methods = _methods;
    });
    ...
    pluginCompat.tap(compiler, '_hardSourceMethods', 'HardDependencyBlockPlugin', methods => {
      // store = methods.store;
      // fetch = methods.fetch;
    ...
    pluginCompat.tap(compiler, '_hardSourceFreezeDependencyVariable', 'HardDependencyBlockPlugin', (frozen, {name, expression, dependencies}, extra) => ({
    ...
    pluginCompat.tap(compiler, '_hardSourceFreezeDependencyBlock', 'HardDependencyBlockPlugin', (frozen, block, extra) => {
    ...
    pluginCompat.tap(compiler, '_hardSourceThawDependencyVariable', 'HardDependencyBlockPlugin', (variable, {name, expression, dependencies}, extra) => new DependenciesBlockVariable(
      name,
      expression,
      mapThaw('Dependency', null, dependencies, extra)
    ));

    pluginCompat.tap(compiler, '_hardSourceThawDependencyBlock', 'HardDependencyBlockPlugin', (block, frozen, extra) => {
      extra.schemas = schemas;
