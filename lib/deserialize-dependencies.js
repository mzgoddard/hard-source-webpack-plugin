...
function deserializeDependencies(deps, parent) {
  return deps.map(function(req) {
    ...
    if (req.harmonyImportSpecifier) {
      ...
      var dep = new HardHarmonyImportSpecifierDependency(
        this.state.imports[req.harmonyRequest],
        req.harmonyId,
        req.harmonyName
      );
      dep.loc = req.loc;
    ...
    if (req.harmonyExportImportedSpecifier) {
      ...
      var activeExports = req.harmonyActiveExports;
      if (!!Set && Array.isArray(activeExports)) {
        activeExports = new Set(activeExports);
      }

      var otherStarExports = req.harmonyOtherStarExports;
      if (Array.isArray(otherStarExports)) {
        otherStarExports = otherStarExports.map(dep => deserializeDependencies([dep], parent));
      }
