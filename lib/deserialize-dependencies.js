...
function deserializeDependencies(deps, parent) {
  return deps.map(function(req) {
    ...
    if (req.harmonyImport) {
      ...
      return this.state.imports[req.request] = new HardHarmonyImportDependency(req.request, req.importedVar, req.range);
    }
    if (req.harmonyImportSpecifier) {
      if (!this.state.imports[req.harmonyRequest]) {
        this.state.imports[req.harmonyRequest] = new HardHarmonyImportDependency(req.harmonyRequest, req.harmonyImportedVar, req.harmonyRange);
      }
